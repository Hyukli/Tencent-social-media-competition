...
#分别链接test/train和ad、position两个表
user_train=pd.merge(user_train,ad,how='left')
user_test=pd.merge(user_test,ad,how='left')
user_train=pd.merge(user_train,position,how='left')
user_test=pd.merge(user_test,position,how='left')

print user_train[:10]
print user_test[:10]

#挑选合适的特征,这里挑选了所有的特征
selected_feas=['clickTime','userID','creativeID','positionID','connectionType','telecomsOperator','age','gender','education','marriageStatus','adID','camgaignID','advertiserID','appID','appPlatform','sitesetID','positionType']
...
rfc_submission.to_csv('database/join_rfc_submission2.csv',index=False)
print '最后赋值结束'
